    ...
    async fetchChats(userId: number | undefined) {
        try {
            const chats = await prismaService.chat.findMany({
                where: {
                    OR: [  // Include chats where the user is a member or group chats where the user is the admin
                        {
                            users: {
                                some: {
                                    id: userId
                        ...
                        {
                            groupAdminId: userId  // Include group chats where the user is the admin
                        }
                    ]
                },
                include: {
